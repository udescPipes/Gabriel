...
#PAra a análise, será desconsiderado os filmes que não possui receita e orçamento mencionado
dados <- dados %>%
filter(!is.na(receita), !is.na(orcamento), !is.na(nota_imdb))
...
renderPlotly({
  ...
  aux <- filmes_produtora %>% 
    group_by(ano) %>% 
     summarise(
       qtd_filmes = n(),
       receita_anual = sum(lucro, na.rm = T)
    )
    
  plot_ly(aux, type = "scatter", fill = 'tozeroy') %>%
    add_lines(x = ~ano, y = ~receita_anual) %>%
    layout(
